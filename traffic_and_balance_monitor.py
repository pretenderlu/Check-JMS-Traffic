...
def get_siliconflow_balance():
    """查询硅基流动账户余额"""
    if not SILICONFLOW_API_KEY:
        print("未设置SILICONFLOW_API_KEY，无法查询余额")
        return None
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if user_info.get('data'):
                data = user_info['data']
                gift_balance = data.get('balance', '0.00')  # 赠送余额
                charge_balance = data.get('chargeBalance', '0.00')  # 充值余额
                total_balance = data.get('totalBalance', '0.00')  # 账户总余额
                return {
                ...
                print("硅基流动余额信息不可用或返回格式错误")
                return None
        else:
            print(f"硅基流动余额查询失败，状态码: {response.status_code}")
            return None
    except Exception as e:
        print(f"硅基流动余额查询发生错误: {str(e)}")
        return None
...
- 充值余额: **{topped_up_balance} CNY**
            """
            print("DeepSeek余额检查成功:")
            print(balance_message)
            final_message_content += balance_message
        except Exception as e:
            print(f"处理DeepSeek余额数据时发生错误: {str(e)}")
            final_message_content += "DeepSeek余额数据处理出错。\n"
    else:
        print("无法获取DeepSeek余额数据。")
        final_message_content += "无法获取DeepSeek余额数据。\n"

    # 查询硅基流动余额
    siliconflow_balance_info = get_siliconflow_balance()
    if siliconflow_balance_info:
        try:
            gift_balance = siliconflow_balance_info['gift_balance']
            charge_balance = siliconflow_balance_info['charge_balance']
            total_balance = siliconflow_balance_info['total_balance']
            sf_balance_message = f"""
## 硅基流动账户余额
- 账户总余额: **{total_balance} CNY**
- 赠送余额: **{gift_balance} CNY**
- 充值余额: **{charge_balance} CNY**
            """
            print("硅基流动余额检查成功:")
            print(sf_balance_message)
            final_message_content += sf_balance_message
        except Exception as e:
            print(f"处理硅基流动余额数据时发生错误: {str(e)}")
            final_message_content += "硅基流动余额数据处理出错。\n"
    else:
        print("无法获取硅基流动余额数据。")
        final_message_content += "无法获取硅基流动余额数据。\n"
